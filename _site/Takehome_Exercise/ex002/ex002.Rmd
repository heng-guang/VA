...
We can now create a basic interactive chart that shows the population of Males/Females in Singapore in a certain year based on the age groups.

### Plotting the Data

First, we assign a tooltip to what we want to show on text hover, in this case we want to show the Population of an Age Group in a specific Year.

```{r}
name <- paste0("\n", total_popn$Sex, 
               " Population of Ages ", total_popn$AG,
                      "\nin the Year ", total_popn$Time, 
                      "\n= ", total_popn$Population)
```

We then use the same code as in [Take-home Exercise 1](https://hengguang-608.netlify.app/takehome_exercise/ex001/ex001), while changing the **Title** and **Subtitle** to ensure clarity. We also add a **label = name** in the mapping to show the tooltip later.

In this plot, we also pass through a colour through the geom_col function to clearly show the separation of Years.

```{r}
p <- ggplot(data = total_popn,
       mapping = aes(
...
Next, we use the **ggplotly** function of the *plotly* package and change the default tooltip (which shows every mapped value) to the tooltip we want with the code chunk below.

```{r}
ggplotly(p, tooltip = c("label"))
```

We can now interact with the chart, and it shows the data that we want to show on hover, great! Other interactive elements include the ability to filter Male and Female population only. We can also zoom in on the chart if there is a particular data point that we are interested in, as shown below.
...
Next, we want an animated chart to show the changes in the population pyramid structure over time. 
...
Lastly, we will use the **transition_time** and **ease_aes** functions of the *gganimate* package to show how the data changes over time, and pass the **Time** column (which shows the years) as an animation.
